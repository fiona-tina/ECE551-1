...
board_t * makeBoard(int w, int h, int numMines) {
  //WRITE ME!
  board_t * thisBoard = mallocThis(sizeof *thisBoard);
  thisBoard->width = w;
  thisBoard->height = h;
  thisBoard->totalMines = numMines;
  thisBoard->board = mallocThis(h * sizeof(thisBoard->board[0]));
  for (int i = 0; i < h; i++) {
    thisBoard->board[i] = mallocThis(w * sizeof(thisBoard->board[0][0]));
    for (int j = 0; j < w; j++) {
      thisBoard->board[i][j] = UNKNOWN;
    }
  }
  int k = 0;
  while (k < numMines) {
    addRandomMine(thisBoard);
    k++;
  }
...
int checkValidCord(int r, int c, int row, int col) {
  if (r < 0 || c < 0)
    return 0;
  if (r >= row || c >= col)
    return 0;
  ...
  for (int i = y - 1; i <= y + 1; i++) {
    for (int j = x - 1; j <= x + 1; j++) {
      if (i == y && j == x)
        ...
        if (checkValidCord(i, j, b->height, b->width)) {
